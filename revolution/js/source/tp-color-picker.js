...
window.RevColor = {
    ...
    convert: function (color, opacity) {
        ...
        try {
            if (color.search("#") !== -1 || color.length < 8) {
                ...
                return RevColor.processRgba(
                    RevColor.sanitizeHex(color),
                    opacity,
                );
            } else {
    ...
    rgbaToHex: function (clr) {
        ...
        return [
            RevColor.getRgbToHex(values[0], values[1], values[2]),
            values[3],
        ];
    },
    ...
    processGradient: function (obj) {
        var tpe = obj.type,
            begin = tpe + "-gradient(",
            middle =
                tpe === "linear" ? obj.angle + "deg, " : "ellipse at center, ",
            colors = obj.colors,
            len = colors.length,
            end = "",
            clr;
...
(function ($) {
    ...
    function writePresets(container, colors) {
        var frag = document.createDocumentFragment(),
            ...
            len = colors.length,
            minRows =
                container.search("colors") !== -1 ? minColorRows : minGradRows,
    ...
    function onChange(gradient, color, trans) {
        ...
        if (!gradient) {
            var hex = color || colorHex.val(),
                opacity =
                    typeof trans !== "undefined" ? trans : colorOpacity.val();
    ...
    function getRgbVal(clr, cloned) {
        var opacity = !cloned ? clr.a : "1";
        return (
            "rgba(" + clr.r + "," + clr.g + "," + clr.b + "," + opacity + ")"
        );
    }
    ...
    function checkGroup() {
        ...
        if (len > presetColumns)
            $("#" + this.id + "-btn").addClass("multiplerows");
    ...
    function checkRows() {
        var $this = $(this),
            minRows =
                this.id.search("colors") !== -1 ? minColorRows : minGradRows,
        ...
        for (var i = 0; i < rows; i++) {
            var start = i * presetColumns,
                range = colors.slice(
                    start,
                    parseInt(start + presetColumns, 10) - 1,
                );
    ...
    function onDragStart() {
        var $this = $(this);
        dragPoint = $this.hasClass("rev-cpicker-point-bottom")
            ? "bottom"
            : "top";
        $this.click();
    }
    ...
    function updateSlider(opacity) {
        ...
        opacitySlider.slider(
            "value",
            Math.round(opacity * 0.01 * sliderHeight),
        );
    ...
    function wheelMove(e) {
        ...
        var val =
            Math.atan2(posY - centerWheel, posX - centerWheel) *
                (180 / Math.PI) +
            90;
        if (val < 0) val += 360;
    ...
    function init() {
            ...
            if (cPicker.hasClass("open")) {
                var targt = e.target,
                    ...
                    isTarget =
                        ...
                        ids.search("hit") !== -1 ||
                        $this.closest(
                            "#rev-cpicker-section-right, #rev-cpicker-point-wrap",
                        ).length;
        ...
        $(".rev-cpicker-point-location")
            .on("keyup.revcpicker focusout.revcpicker", function (e) {
                ...
                if (isNaN(point))
                    point =
                        evt === "keyup" ? "0" : curPoint.attr("data-location");
        ...
        $("body")
            ...
            .on("change.revcpicker", ".rev-cpicker-component", function () {
                var $this = $(this),
                    val =
                        $this.data("gradient") || $this.val() || "transparent";

                if (val === "transparent" || RevColor.transparentRgba(val))
                    val = "";
            ...
            .on(
                ...
                function () {
                    if (selectedColor) {
                        if (
                            selectedColor[0] === this &&
                            selectedColor.hasClass("selected")
                        )
                            return;
                        selectedColor.removeClass("selected");
                    }
                    ...
                    if (btn.hasClass("active"))
                        btn.find(".rev-cpicker-arrow-up").click();
                    if (cPicker.hasClass("color-view")) {
                        var val = selectedColor.attr("data-color");

                        supressColor = true;
                        colorIris.val(val).change();
                        if (colorHex.val() === "transparent")
                            colorHex.val(val.toLowerCase());
                        supressColor = false;
            ...
            .on(
                ...
                function () {
                    if (gradientsOpen)
                        gradOutput.style.background = getAttribute(
                            this,
                            "data-color",
                        );
                },
            ...
            .on("click.revcpicker", ".rev-cpicker-delete", function () {
                ...
                if (
                    window.confirm(
                        document.getElementById("rev-cpicker-remove-delete")
                            .innerHTML,
                    )
                ) {
                    ...
                    doc.off("revcpicker_onajax_delete.revcpicker").on(
        ...
        $(".rev-cpicker-save-preset-btn").on("click.revcpicker", function () {
            ...
            presetGroup = cPicker.hasClass("color-view")
                ? "colors"
                : "gradients";
            titl = $.trim(titl.replace(/\W+/g, "_"))
                .replace(/^\_|\_$/g, "")
            ...
            if (presetGroup === "colors") {
                ...
                color = $.extend(
                    {},
                    buildGradientOutput(false, false, true)[0],
                );
            }

            newColorValue[titl] = color;
            doc.off("revcpicker_onajax_save.revcpicker").on(
        ...
        directions.on("click.revcpicker", function () {
            ...
            if (dir !== "radial")
                angle.removeAttr("disabled").val(getDegree(dir));
        ...
        $(".rev-cpicker-opacity-input")
            .on("keyup.revcpicker focusout.revcpicker", function (e) {
                ...
                if (isNaN(opacity))
                    opacity =
                        evt === "keyup" ? "0" : $(this).data("orig-value");
        ...
        $(".rev-cpicker-builder-hit").on("click.revcpicker", function (e) {
            ...
            var hit = parseInt(
                    ((e.pageX - hitTop.offset().left) / hitWidth).toFixed(2) *
                        100,
                    10,
                ),
            ...
            if (align === "bottom") {
                if (
                    cPicker.find(".rev-cpicker-point-bottom").length < maxPoints
                ) {
    ...
    $.tpColorPicker = function (settings) {
        ...
        if (settings.defaultValue)
            RevColor.defaultValue = settings.defaultValue;
    ...
    var ColorPicker = {
        refresh: function () {
            ...
            if ($this.hasClass("rev-cpicker-component")) {
                ...
                colorValue =
                    colorValue[1] !== "rgba" ||
                    !RevColor.transparentRgba(val, true)
                        ? val
                        : "";
            ...
            if (
                ...
                if (
                    ...
                    presets.core = {
                        ...
                        gradients:
                            dataGradients || settingsGradients || defGradients,
                    };
                }
